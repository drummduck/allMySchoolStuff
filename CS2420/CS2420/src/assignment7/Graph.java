...
public class Graph 
{
   ...
   * distance from s". In order to associate each vertex with a priority
   * (related to its distance from s), you must either use the Comparable
   * interface or the Comparator interface. See the API for PriorityQueue, and
   * ask the course staff if you need help.
   */
...
//    if (start == null)
//      throw new UnsupportedOperationException("Vertex " + startName
//          + " does not exist.");
//
//    // set up priority queue of vertices prioritized by smallest distance from
...
//	  {
//        // get the destination vertex for the edge and consider the cost of this
//        // path
//        Vertex dest = e.getOtherVertex();
//        double cost = e.getWeight() + x.getDistanceFromStart();
//
//        // if the cost of this path is smaller than the previously considered
//        // path, update
//        if (dest.getDistanceFromStart() > cost) 
//		{
//          dest.setDistanceFromStart(cost);
//          dest.setPrevious(x);
//          // if the destination vertex has not been visited, add to the priority
//          // queue
//          if (!dest.getVisited())
//            pq.offer(dest);
//        }
...
//    for (Vertex v : vertices.values()) 
//	{
//      System.out.print("Shortest path from " + start.getName() + " to "
//          + v.getName() + " has length " + v.getDistanceFromStart()
//          + " and is ");
//      String s = "";
