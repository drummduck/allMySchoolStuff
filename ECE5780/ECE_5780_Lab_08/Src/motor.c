...
void pwm_init(void) {
    ...
    /// TODO: Set up a pin for H-bridge PWM output (TIMER 16 CH1)
		...
		//GPIOA->MODER |= (0x2<<12);										// PA6 set to Alternate Function.
		//GPIOA->AFR[0] |= (0x5<<24);										// PA6 set to AF5.
	
		RCC->AHBENR |= RCC_AHBENR_GPIOBEN;
		GPIOB->MODER |= (0x2<<16);										// PB8 set to Alternate Function.
		GPIOB->AFR[1] |= (0x2<<0);										// PB8 set to AF2.	
	
    
    /// TODO: Set up a few GPIO output pins for direction control
		/// TODO: Initialize one direction pin to high, the other low
    GPIOA->MODER |= (1<<8)|(1<<10);								// PA4,PA5 set to O/P.
		GPIOA->ODR |= (1<<4);													// PA4 to High.
...
void encoder_init(void) {
    ...
    RCC->AHBENR |= RCC_AHBENR_GPIOBEN;
		GPIOB->MODER  |= (0x2<<8)|(0x2<<10);								//PB4,PB5 to Alt. Function.
		GPIOB->AFR[0] |= (0x1<<16)|(1<<20);									//PB4,PB5 to AF1. PB4 is CH1, PB5 is CH2.
...
void PI_update(void) {
    ...
    int16_t output = error_integral + (Kp * error); 		// Change this!
