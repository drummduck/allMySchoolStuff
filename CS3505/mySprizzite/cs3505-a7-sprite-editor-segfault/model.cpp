...
//The model class holds all information to display on the GUI. It is updated through signals activated through
//the GUI to Sprizzite to here. The model holds booleans for the state of the system, data structures that keep
//track of pixels, multiple canvas', and other variables.
...
//Creates a sprite with user given dimensions. The canvas, preview, viewport and frames are set and booleans
//are set to the proper state. Scaling is done for the frames and preview.
...
//Given a data structure of sprites from an already parsed file(in Sprizzite). The canvas, preview, viewport and
//frames are set and booleans are set to the proper state. Scaling is done for the frames and preview.
 void Model::fileOpen(std::vector<Sprite> inMsprites)
{   //Ignore timer to avoid conflicts
    MtimeIgnore = true;
    if(MstartedProject == false)
        {
        //Clear data structures and fill sprites.
        Msprites.clear();
        MpreviewMaps.clear();
        Msprites = inMsprites;
        MactiveSprite = Msprites.size()-1;
        ...
        //initiates viewport
        Mvp = new viewport(0, 0, McanvasWidth/Mcanvas->getPixelViewSize(), McanvasHeight/Mcanvas->getPixelViewSize());
        //Figure out the scaling for the preview and frames.
        int size;
        if(Msprites[MactiveSprite].getWidth() > Msprites[MactiveSprite].getHeight())
            {
            size = Msprites[MactiveSprite].getWidth();
        ...
        //Fill preview data structure with file information
        ...
        //Set the current and previous frames in the frames carousel
    ...
    //Unignore timer to continue updating preview
    MtimeIgnore = false;
}
...
void Model::fillCanvas()
{
    ...
    if(Mselected)
        ...
        Mcanvas->drawSelectRect((Mselection->getLeft() - Mvp->getLeft())*pvs,
                                (Mselection->getTop()- Mvp->getTop())*pvs,
                                (Mselection->getRight() - Mvp->getLeft())*pvs,
                                (Mselection->getBot() - Mvp->getTop())*pvs);
        MselectingRectBot = 0;
        MselectingRectLeft = 0;
        MselectingRectRight = 0;
        MselectingRectTop = 0;
    ...
    //Unignore time to continue preview update
    MtimeIgnore = false;
}
        ...
        for( int b = 0; b < (pensize); b = b + 1 ) {
            if((y-b) > 0 && (y-b) < Msprites[MactiveSprite].getHeight()){
                if((x-a) > 0 && (x+a) < Msprites[MactiveSprite].getWidth()){
                    ...
                    MpreviewMaps[MactiveSprite]->drawPixel((Xpos - Xpos % MpreviewMaps[MactiveSprite]->getPixelViewSize())+a, (Ypos - Ypos % MpreviewMaps[MactiveSprite]->getPixelViewSize())-b, pencolor);
        ...
        for( int b = 0; b < (pensize); b = b + 1 ) {
            if((y-b) > 0 && (y-b) < Msprites[MactiveSprite].getHeight()){
                if((x+a) > 0 && (x+a) < Msprites[MactiveSprite].getWidth()){
                    ...
                    MpreviewMaps[MactiveSprite]->drawPixel((Xpos - Xpos % MpreviewMaps[MactiveSprite]->getPixelViewSize())+a, (Ypos - Ypos % MpreviewMaps[MactiveSprite]->getPixelViewSize())-b, MsecondaryColor);
...
//Frame will be added to end of frame caraousel and will make that the active frame.
//Previous frame is also updated as well.
void Model::addFrameButton()
{
    //Ignore time to avoid conflicts
    MtimeIgnore = true;
    //Update current and previous frames in GUI
    ...
    //Unignore time and mark the project as changed
    MtimeIgnore = false;
    Mchanged = true;
}

//Delete current frame and slide previous frame to current frame.
//If no left frames, slide next frame to current frame.
void Model::frameDeleteButton()
{
    ...
    if(MstartedProject == true)
        ...
        //Last frame, slide previous to current
        ...
        //Normal delete, slide previous to current or slide next to current(depending on case)
        else
            {
                PixelCanvas *tempCurrent = new PixelCanvas(McurrentSize, McurrentSize, MscaleCurrent);
    ...
    //Unignore time to continue updating preview, mark project as changed
    MtimeIgnore = false;
    Mchanged = true;
}

//Returns the current frame we are on
...
void Model::fillSelection() {
    ...
    for(int i = Mselection->getLeft(); i < Mselection->getRight(); i++) {
        for(int j = Mselection->getTop(); j < Mselection->getBot(); j++) {
            ...
            //for some reason drawing each pixel is more laggy than filling Mcanvas
            //Mcanvas->drawPixel(i*Mcanvas->getPixelViewSize(), j*Mcanvas->getPixelViewSize(), primaryColor);
        }
    }
    fillPreviewFromChange(MactiveSprite);
    fillFramesFromFillOrPaste();
...
bool Model::inSelection(int x, int y) {
    return true;
}
...
void Model::paintSelection(int left, int top, int right, int bot) {
    ...
    for (int i = Mvp->getLeft(); i < MhorizPixels; i++) {
        set.push_back(Msprites[MactiveSprite].getPixelColor(i, top));
    }
    ...
    for (int i = Mvp->getLeft(); i < MhorizPixels; i++) {
        ...
        set.push_back(Msprites[MactiveSprite].getPixelColor(i, bot));
    }
    Mcanvas->paintRow(bot, set);
    set.clear();

    //paint left col
    for (int i = Mvp->getTop(); i < MvertPixels; i++) {
        set.push_back(Msprites[MactiveSprite].getPixelColor(left, i));
    }
    Mcanvas->paintColumn(left, set);
    set.clear();

    //paint right col
    for (int i = Mvp->getLeft(); i < MvertPixels; i++) {
        if (i == Mvp->getBot()) break;
        set.push_back(Msprites[MactiveSprite].getPixelColor(right, i));
    }
    Mcanvas->paintColumn(right, set);
    set.clear();
}

//Store all sprites in preview data structure
void Model::fillPreviewFromOpenAndNew(int spriteNumber, PixelCanvas * previewCanvas)
{
    //Unignore time and grab current sprite
    MtimeIgnore = true;
    std::vector<QColor> row;
    Sprite previewSprite = Msprites[spriteNumber];
    //paint sprite on to canvas
    ...
    //store canvas and continue preview updating
    MpreviewMaps.push_back(previewCanvas);
    MtimeIgnore = false;
}

//Change current sprite and update current canvas with sprite
void Model::fillPreviewFromChange(int previewSprite)
{
    ...
    //Paint canvas with updated sprite
    ...
    //Update canvas and continue preview updating
    MpreviewMaps[previewSprite] = tempCanvas;
    MtimeIgnore = false;
}

//Push previous canvas onto the data structure, creating copy of last frame.
...
//Update current and previous frames from an Open file
void Model::fillFramesFromOpen(int spriteNumber, PixelCanvas *current, PixelCanvas *previous)
{
    ...
    //Update current canvas with current sprite.
    ...
    //This is for files bigger than 1 sprite
    if(Msprites.size() > 1)
        {
        //Update previous canvas with previous sprite
        ...
        //next canvas will be transparent because nothing is there.
    ...
    //For case that we open a file that has 1 sprite
    else
        {
        //Update current canvas with current sprite and set the previous and next canvas to transparent.
        PixelCanvas *tempNext = new PixelCanvas(MotherSize, MotherSize, MscaleOther);
        PixelCanvas *tempPrevious = new PixelCanvas(MotherSize, MotherSize, MscaleOther);
    ...
    //unignore time to continue preview updating
    MtimeIgnore = false;
}

//Updates current and previous frame canvas' when adding another sprite
void Model::fillFramesFromAdd(int spriteNumber, PixelCanvas *currentCanvas, PixelCanvas *previousCanvas)
{
    ...
    std::vector<QColor> row;
    Sprite frameSprite = Msprites[spriteNumber];
    //Update current frame canvas with current sprite
    for(int i = 0; i < frameSprite.getHeight(); i++)
        {
        for(int j = 0; j < frameSprite.getWidth(); j++)
            {
    ...
    //Update previous frame canvas with previous sprite
    frameSprite = Msprites[spriteNumber-1];
    for(int i = 0; i < frameSprite.getHeight(); i++)
        {
        for(int j = 0; j < frameSprite.getWidth(); j++)
    ...
    //Set next canvas to transparent
    PixelCanvas *tempCanvas = new PixelCanvas(MotherSize, MotherSize, MscaleOther);
    QPixmap *tempMap = tempCanvas->getMap();
    tempMap->fill(Qt::transparent);
    emit updateNext(*tempCanvas->getMap());
...
//Clear last frame to a blank canvas
void Model::fillFramesFromDeleteLast(PixelCanvas *newCanvas)
{
    ...
    //Update current canvas with current sprites
    for(int i = 0; i < frameSprite.getHeight(); i++)
        {
        for(int j = 0; j < frameSprite.getWidth(); j++)
            {
...
//Delete frames when in last position
void Model::fillFramesFromDeleteEnd(PixelCanvas *currentCanvas, PixelCanvas *previousCanvas)
{
    ...
    //Update current canvas with previous sprite
   for(int i = 0; i < frameSprite.getHeight(); i++)
        {
        for(int j = 0; j < frameSprite.getWidth(); j++)
            {
    ...
    //If previous sprite is the last one, make previous canvas transparent
    if(MactiveSprite-1 == 0)
        {
        QPixmap *tempMap = previousCanvas->getMap();
        tempMap->fill(Qt::transparent);
        }
    //If previous sprite is not the last one, update with frame before previous frame
    else
        {
...
//Last case of delete, normal delete, slide previous to right and also one before previous to right
//Also case of being on front canvas and next on right is last
void Model::fillFramesFromDelete(PixelCanvas* current, PixelCanvas* next, PixelCanvas* previous)
{
    ...
    //Case for very first canvas and more on right
    else if(MactiveSprite+1 != Msprites.size()-1 && MactiveSprite == 0)
        {
        frameSprite = Msprites[MactiveSprite+1];
        //Update current canvas with next sprite
        for(int i = 0; i < frameSprite.getHeight(); i++)
            {
            for(int j = 0; j < frameSprite.getWidth(); j++)
                {
        ...
        //Update next canvas with sprite in front of next
        for(int i = 0; i < frameSprite.getHeight(); i++)
            {
            for(int j = 0; j < frameSprite.getWidth(); j++)
                {
    ...
    //Need case for current frame and previous is last canvas
    else if(MactiveSprite-1 == 0)
        {
        frameSprite = Msprites[MactiveSprite-1];
        //Current canvas updated from previous sprite
        for(int i = 0; i < frameSprite.getHeight(); i++)
            {
            for(int j = 0; j < frameSprite.getWidth(); j++)
                {
        ...
        //Previous canvas set to transparent
        QPixmap *tempMap = previous->getMap();
        tempMap->fill(Qt::darkGray);
        emit updateCurrent(*current->getMap());
        emit updatePrevious(*previous->getMap());
        }
    //need case for deleting middle and left moves over, regular case
    else
        {
        frameSprite = Msprites[MactiveSprite-1];
        //Update current canvas with previous sprite
        ...
        frameSprite = Msprites[MactiveSprite-2];
        //Update previous canvas with 2 sprites to the left
        for(int i = 0; i < frameSprite.getHeight(); i++)
            {
            for(int j = 0; j < frameSprite.getWidth(); j++)
                {
    ...
    //Unignore time to continue updating preview
    MtimeIgnore = false;
}

//This method updates all frames for all cases of sliding frames left
void Model::nextFrame()
{
    ...
    //case for if the next one on right is last one OR the left is empty and right will be empty.
        ...
        //Update previous canvas with current sprite
        for(int i = 0; i < frameSprite.getHeight(); i++)
            {
                for(int j = 0; j < frameSprite.getWidth(); j++)
                    {
    ...
    //This is for a normal delete
    else
        {
        frameSprite = Msprites[MactiveSprite+2];
        //Update next canvas with sprite two slots to right
        for(int i = 0; i < frameSprite.getHeight(); i++)
            {
            for(int j = 0; j < frameSprite.getWidth(); j++)
                {
...
//Slide frames from left to right
void Model::previousFrame()
{
    MtimeIgnore = true;
    std::vector<QColor> row;
    Sprite frameSprite = Msprites[MactiveSprite];
    //Setup canvas' for all frames
    PixelCanvas *tempCurrent = new PixelCanvas(McurrentSize, McurrentSize, MscaleCurrent);
    PixelCanvas *tempNext = new PixelCanvas(MotherSize, MotherSize, MscaleOther);
    PixelCanvas *tempPrevious = new PixelCanvas(MotherSize, MotherSize, MscaleOther);
    tempCurrent->toggleGrid(false);
    ...
    //case for if the next one to left is last one OR the right one is empty and the left one will be empty.
        ...
        {
        frameSprite = Msprites[MactiveSprite-1];
        //Current canvas updated with previous sprite
        for(int i = 0; i < frameSprite.getHeight(); i++)
            {
            for(int j = 0; j < frameSprite.getWidth(); j++)
                {
        ...
        frameSprite = Msprites[MactiveSprite];
        //Next canvas updated with current sprite
        for(int i = 0; i < frameSprite.getHeight(); i++)
            {
            for(int j = 0; j < frameSprite.getWidth(); j++)
                {
        ...
        //Set previous canvas to transparent
    ...
    //case for normal previousframe
    else
    {
    frameSprite = Msprites[MactiveSprite-2];
    //Update previous canvas with sprite to slots left
    for(int i = 0; i < frameSprite.getHeight(); i++)
        {
        for(int j = 0; j < frameSprite.getWidth(); j++)
            {
    ...
    frameSprite = Msprites[MactiveSprite];
    //Update next canvas with current sprite
    for(int i = 0; i < frameSprite.getHeight(); i++)
        {
        for(int j = 0; j < frameSprite.getWidth(); j++)
            {
...
//Fills the current frame from a new file action
void Model::fillFramesFromNew(PixelCanvas* current, PixelCanvas* others)
{
    ...
    std::vector<QColor> row;
    Sprite frameSprite = Msprites[MactiveSprite];
    //Set current canvas with current sprite
    for(int i = 0; i < frameSprite.getHeight(); i++)
        {
        for(int j = 0; j < frameSprite.getWidth(); j++)
            {
    ...
    //Set next and previous canvas' to transparent
...
//Update current canvas from live updates
