...
//Sprizzite controls all interaction between the data structures in the Model class and the GUI.
//It sends signals between Model and FileDialog and also has some prompting.
    ...
    ui(new Ui::Sprizzite)
{
    ...
    //Start new file/resize/canvas update
...
bool Sprizzite::eventFilter(QObject *obj, QEvent *event)
{
    ...
    if(obj == ui->Canvas) {
        if (event->type() == QEvent::MouseMove)
        {
          ...
          //PixelCanvas::draw_pixel(mouseEvent->pos().x(),mouseEvent->pos().y(),);
...
//Prompt before opening or starting a new project to check if user wants to save or not.
...
//Overwritten close even for window, ask for save before exiting if project was changed
...
//Update text on frame caraousel when delete is pushed
...
void Sprizzite::exportGif() {
    ...
    if (fileName.isEmpty()) {
       ...
       // get relevant data from Mmodel, write to file.
        ...
        GifWriter writer; //Start writing to a gif file
...
void Sprizzite::saveSprite() {
    ...
    //If project hasnt started, you cant save
    if(Mmodel.MstartedProject == false)
        {
        return;
        }
     QString fileName;
    //Use stored save file name if saving
    ...
    //Set project as not changed and saved
    emit changed();
    emit saved();

}

void Sprizzite::OpenDialog() { //TODO: make error safe.
    //get target file - this is a full path to the file, we should
    //be able to open it.
  ...
  if(Mmodel.Mchanged)
    ...
    std::cout << MsavedFileName.toStdString() << std::endl;
    ...
    //Set project as started/saved and update frames GUI
    emit startFromOpen();
    emit saved();
