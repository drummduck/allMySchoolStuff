    ...
    ui(new Ui::Sprizzite)
{
    ...
    //Open File
    QObject::connect(this, SIGNAL(fileLoad(QString,int,int,int,std::vector<Sprite>)), &Mmodel, SLOT(fileOpen(QString,int,int,int,std::vector<Sprite>)));
...
void Sprizzite::OpenDialog() {
  ...
  if(Mmodel.Mchanged)
    ...
    emit fileLoad(Name, W, H, numFrames, Frames);
