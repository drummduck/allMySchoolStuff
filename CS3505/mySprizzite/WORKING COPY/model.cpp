...
void Model::createNewFile(int w, int h)
{
 ...
 void Model::fileOpen(QString name, int width, int height, int frames, std::vector<Sprite> inMsprites)
{
     MtimeIgnore = true;
...
void Model::fillCanvas()
{
    ...
    MtimeIgnore = true;
    //TODO:QPixmap might be useful to more than just the preview.
    //Use active sprite
    Sprite s = Msprites[MactiveSprite];
    Mcanvas->clear();
